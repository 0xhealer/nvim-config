-- skeleton.lua (C version)
-- Adapted from SalarAlo/neovim_configuration's lua/salar/tools/skeleton.lua.
--
-- Everything before M.insert() is copied verbatim — the header/source
-- matching logic (finding a .c's corresponding .h, scoring candidates,
-- computing the right #include path via compile_commands.json or
-- conventional include/src layout) has no C++-specific content at all, it
-- works identically for C.
--
-- Only M.insert() changed: dropped namespace/class wrapping (C has neither)
-- and the iostream-based main.cpp template, replaced with stdio.h/printf.
-- HEADER_EXTENSIONS trimmed to just "h" since .hpp/.hh/.hxx aren't C.
...
function M.insert()
	...
	if is_header_extension(ext) then
		-- No namespace/class wrapping — C has neither. Just a clean, empty
		-- include-guarded header ready for prototypes/typedefs/structs.
		lines = {
			"#pragma once",
			"",
		}
